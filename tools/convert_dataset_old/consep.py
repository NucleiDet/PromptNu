...
def parse_single_item(item, raw_image_folder, raw_label_folder, new_path, crop_size):
    ...
    instance_label, type_label = convert_mat_to_array(label_path)
    ...
    if crop_size != 0:
        ...
        type_patches = crop_patches(type_label, crop_size)
        ...
        type_patches = [type_label]
        semantic_patches = [semantic_label]
        # record patch item name
        sub_item_list = [item]

    # patch storage
    patch_batches = zip(image_patches, instance_patches, type_patches, semantic_patches)
    for patch, sub_item in zip(patch_batches, sub_item_list):
        # jump when exists
        if osp.exists(osp.join(new_path, sub_item + '.png')):
            continue
        # save image
        pillow_save(osp.join(new_path, sub_item + '.tif'), patch[0])
        ...
        palette[0, :] = (0, 0, 0)
        palette[1, :] = (255, 255, 2)
        pillow_save(osp.join(new_path, sub_item + '_type.png'), patch[2])
        pillow_save(osp.join(new_path, sub_item + '_type_color.png'), colorize_seg_map(patch[2]))
        pillow_save(osp.join(new_path, sub_item + '_sem.png'), patch[3], palette)
