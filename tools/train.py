...
def parse_args():
    ...
    parser.add_argument('--config', default='configs/promptnu/promptnu_adam-lr1e-4_bs8_256x256_300e_consep.py', help='train config file path')
    parser.add_argument('--exp_name',default='debug', help='')
...
def main():
    ...
    if args.work_dir is not None:
        ...
        cfg.work_dir = f'./work_dirs/{model_name}/{config_name}/{args.exp_name}'
    ...
    # set random seeds
    fixed_seed = 42
    seed = init_random_seed(seed=fixed_seed)
    ...
    model = build_segmentor(cfg.model, train_cfg=cfg.get('train_cfg'), test_cfg=cfg.get('test_cfg'), gpu_ids=cfg.gpu_ids)
    model.init_weights()
    ...
    if not distributed:
        ...
        model = revert_sync_batchnorm(model) # 转变为普通的BN
