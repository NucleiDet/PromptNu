...
class CDNet(BaseSegmentor):
    ...
    def __init__(self, num_classes, train_cfg, test_cfg):
        ...
        # out_indices 从 TorchVGG16BN 模型中获取输出的层的索引。这些索引对应于 TorchVGG16BN 中定义的 output_names 属性，即 ('5', '12', '22', '32', '42', '43')。
        ...
        self.head = CDHead(
            ...
            dgm_dims=64,
            bottom_in_dim=512,
            skip_in_dims=(64, 128, 256, 512, 512), # 跳跃连接层的各个层的维度大小
            stage_dims=[16, 32, 64, 128, 256], # 每个阶段（stage）的维度大小
    ...
    def forward(self, data, label=None, metas=None, **kwargs):
        ...
        if self.training:
            ...
            loss.update(training_metric_dict) # 算的loss
            return loss
        else:
        ...
        for rotate_degree in self.rotate_degrees:
            for flip_direction in self.flip_directions:
                ...
                if self.test_cfg.mode == 'split':
                    ...
                    sem_logit, dir_logit, point_logit = self.whole_inference(img, meta, rescale) # 采用滑动窗口的方式，将图像划分为较小的区域，并在这些区域上独立进行推理。
