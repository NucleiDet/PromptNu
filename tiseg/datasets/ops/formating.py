...
def format_global(glo_, dc=True):
    # glo_ = to_tensor(glo_)
    if dc:
        glo_ = DC(glo_, cpu_only=True)
...
class Formatting(object):
    ...
    def __call__(self, data):
        ret = {'data': {}, 'label': {}, 'global_label': {}, 'metas': {}}
        ...
        if data['global_label'] == {}:
            pass
        else:
            for global_key in ['color', 'size', 'density', 'arrange', 'shape']:
                ret['global_label'][global_key] = format_global(data['global_label'][global_key])
