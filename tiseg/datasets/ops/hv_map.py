...
class HVLabelMake(object):
    ...
    def __call__(self, data):
        ...
        inst_color_gt = data['inst_color_gt']
        type_gt = data['type_gt']

        # [H, W, 2]
        hv_gt = gen_instance_hv_map(inst_gt)
        # [2, H, W]
        hv_gt = hv_gt.transpose(2, 0, 1) # (2, 256, 256)

        data['hv_gt'] = hv_gt
        data['inst_color_gt'] = inst_color_gt
        data['type_gt'] = type_gt
        data['seg_fields'].append('hv_gt')
        data['seg_fields'].append('inst_color_gt')
        data['seg_fields'].append('type_gt')
