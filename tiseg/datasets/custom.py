...
from tiseg.utils import (pre_eval_all_semantic_metric, pre_eval_to_sem_metrics, pre_eval_bin_aji, pre_eval_bin_pq,
                         ...
                         pre_eval_to_bin_aji, pre_eval_to_bin_pq, pre_eval_to_F1)
...
class CustomDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 inst_color_suffix='_inst_color.png',
                 type_suffix='_type.png',
                 test_mode=False,
                 split=None):
        ...
        self.inst_color_suffix = inst_color_suffix
        ...
        if self.data_root is not None:
            ...
            if not (self.split is None or osp.isabs(self.split)):
                ...
                self.data_infos = self.load_annotations(self.img_dir, self.ann_dir, self.img_suffix, self.sem_suffix,self.inst_suffix, self.inst_color_suffix, self.type_suffix, self.split)
    ...
    def load_annotations(self, img_dir, ann_dir, img_suffix, sem_suffix, inst_suffix, inst_color_suffix, type_suffix, split=None):
        ...
        if split is not None:
            with open(split, 'r') as fp:
                for line in fp.readlines():
                    ...
                    inst_color_name = img_id + inst_color_suffix
                    type_name = img_id + type_suffix
                    img_file_name = osp.join(img_dir, img_name)
                    sem_file_name = osp.join(ann_dir, sem_name)
                    inst_file_name = osp.join(ann_dir, inst_name)
                    inst_color_file_name = osp.join(ann_dir, inst_color_name)
                    type_file_name = osp.join(ann_dir, type_name)
                    data_info = dict(
                        data_id=osp.splitext(img_name)[0],
                        file_name=img_file_name,
                        sem_file_name=sem_file_name,
                        inst_file_name=inst_file_name,
                        inst_color_file_name=inst_color_file_name,
                        type_file_name=type_file_name,)
                    data_infos.append(data_info)
        else:
            for img_name in mmcv.scandir(img_dir, img_suffix, inst_color_suffix, recursive=True):
                sem_name = img_name.replace(img_suffix, sem_suffix)
                inst_name = img_name.replace(img_suffix, inst_suffix)
                inst_color_name = img_name.replace(img_suffix, inst_color_suffix)
                type_name = img_name.replace(img_suffix, type_suffix)
                img_file_name = osp.join(img_dir, img_name)
                sem_file_name = osp.join(ann_dir, sem_name)
                inst_file_name = osp.join(ann_dir, inst_name)
                inst_color_file_name = osp.join(ann_dir, inst_color_name)
    ...
    def evaluate(self, results, logger=None, **kwargs):
        ...
        # classification F1
        classification_F1_eval_results = ret_metrics.pop('classification_F1')
        results_list_7, results_list_4 = pre_eval_to_F1(classification_F1_eval_results, img_ret_metrics['name'])

        # img_ret_metrics.update(pre_eval_to_F1(classification_F1_eval_results, img_ret_metrics['name']))
        ...
        classification_7_keys = ['F1d', 'Accuracy', 'miscellaneous', 'inflammatory', 'normal epithelial', 'malignant or dysplastic epithelial', 'fibroblast', 'muscle', 'endothelial']
        classification_4_keys = ['F1d', 'Accuracy', 'Fmc-miscellaneous ', 'Fic-inflammatory', 'Fec-epithelial', 'Fsc-spindle-shaped']
        mean_metrics = {}
        overall_metrics = {}
        classification_7_metrics = {}
        classification_4_metrics = {}
        ...
        for i, key in enumerate(classification_7_keys):
            classification_7_metrics[key] = results_list_7[i]
        for i, key in enumerate(classification_4_keys):
            classification_4_metrics[key] = results_list_4[i]
        ...
        # classification_7 table
        classification_7_metrics = OrderedDict(
            {sem_key: np.round(np.mean(value), 3)
             for sem_key, value in classification_7_metrics.items()})

        classification_7_table_data = PrettyTable()
        for key, val in classification_7_metrics.items():
            classification_7_table_data.add_column(key, [val])


        # classification_4 table
        classification_4_metrics = OrderedDict(
            {sem_key: np.round(np.mean(value), 3)
             for sem_key, value in classification_4_metrics.items()})

        classification_4_table_data = PrettyTable()
        for key, val in classification_4_metrics.items():
            classification_4_table_data.add_column(key, [val])
        ...
        print_log('Classification_7 Results:', logger)
        print_log('\n' + classification_7_table_data.get_string(), logger=logger)
        print_log('Classification_4 Results:', logger)
        print_log('\n' + classification_4_table_data.get_string(), logger=logger)
        storage_results = {
            'mean_metrics': mean_metrics,
            'overall_metrics': overall_metrics,
            'classification_7_metrics':classification_7_metrics,
            'classification_4_metrics':classification_4_metrics
        }
